...
val velocities = (
        ...
        "8,-2    27,-8   30,-5   24,-7").split("\n").flatMap { it.split("  ").map { it.trim() } }.filter { it.isNotBlank() }.map {
    val pair = it.split(",")
...
fun main() {
    ...
    fun part2(input: List<String>): Long {
        ...
        val minYVelocity = targetBounds.last().last
        var count = 0L
        val pairs = mutableListOf<Pair<Int, Int>>()
        for (i in minXVelocity..maxXVelocity) {
            for (j in minYVelocity..maxYVelocity) {
                val maxSteps = if (triangleNumbers[i] in targetBounds.first()) i
                else i - triangleNumbers.indexOfFirst { triangleNumbers[i] - it in targetBounds.first() }
                if (j >= 0) {
                    val maxHeight = triangleNumbers[j]
                    val finalHeights = triangleNumbers.map { maxHeight - it }
                    if (maxSteps != i && finalHeights[maxSteps] in targetBounds.last()) {
                        pairs.add(Pair(i, j))
                        count++
                    } else if (maxSteps == i && finalHeights.any { it in targetBounds.last() }) {
                        count++
                        pairs.add(Pair(i, j))
                    }
                } else {
                    if (j in targetBounds.last() && i in targetBounds.first()) {
                        count++
                        pairs.add(Pair(i, j))
                    } else {
                        val finalHeights = triangleNumbers.map { j - it }
                        if (maxSteps != i && finalHeights[maxSteps - 1] in targetBounds.last()) {
                            count++
                            pairs.add(Pair(i, j))
                        }
                    }
...
//            finalYPositions = triangleNumbers.map { maxY - it }
//        }
        val extras = pairs.filter { it !in velocities }
